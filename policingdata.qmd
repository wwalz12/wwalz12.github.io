...
```{r}
#| fig-alt: "A line chart showing the percentage of traffic stops by race in New York State from 2010 to 2018. The line for White drivers is the highest, trending downward from roughly 78% to 70%. The lines for Black and Hispanic drivers show slight upward trends, rising from approximately 10% and 6% respectively to roughly 15% and 9%."
...
```{r}
#| fig-alt: "Two bar charts comparing stop types in Long Beach and San Bernardino. In both cities, 'vehicular' stops are the overwhelming majority, represented by a long bar at the top. Long Beach displays a visible, though smaller, bar for 'pedestrian' stops, whereas San Bernardino reports almost zero pedestrian stops."
...
```{r}
#| fig-alt: "A faceted bar chart showing search and frisk rates by race in Florida. The top bar in both panels corresponds to Black drivers, showing the highest rates for both searches and frisks. Hispanic and White drivers show lower rates. The frisk rates are significantly lower overall (close to 0%) compared to search rates."
